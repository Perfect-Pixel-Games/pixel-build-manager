...
**Execution note (discovered while running this plan):** Rust type-checks the whole crate before running any test, so `cargo test <module>::` cannot pass in isolation while `lib.rs` (not rewritten until Task 3) still calls the old API. Task 1 needed a small compile-only patch to `lib.rs` (see its commit history). Task 2's removals/renames (`active_dir` gone, `find_active_executable`→`find_build_executable`, `ensure_asset_cached` gone) touch nearly every remaining old command in `lib.rs` (`check_release_asset`, `list_cached_assets`, `delete_cached_asset`, `get_active_executable`, `launch_active_build`, `get_active_build_dir`) — at that point a minimal patch *is* essentially Task 3's full rewrite. **Tasks 2 and 3 were therefore executed together, in one implementer dispatch, as two sequential commits** (Task 2's 4 files, then Task 3's `lib.rs` rewrite) rather than two separate dispatches — the intermediate state between those two commits does not compile, which is expected and fine since nothing depends on checking out that exact commit in isolation.
